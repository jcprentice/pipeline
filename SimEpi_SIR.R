...
#
SIRdataGen <- function(
    pop, nReps, nGens, repl, gen, r.g, r.f, TRbeta, RRgamma, SigG.g, SigG.f,
    SigE.g, SigE.f, sires, dpsire, N, gr.size, ngroups, freqDep, reprnum,
    SIRdata
) {
  ...
  # index cases (s==0 is the index case index)
  ...
  # Infected indicator
  # indicator if the individual was infected or not, where s the the index cases
  # (initialisation - at the individual level)
  pop$I <- 1-pop$s 
  
  # tau the "time-to-infection"
  pop[pop$s == 1, "Tinf"] <- -1
  # index case: infected at the start
  pop[pop$s == 0, "Tinf"] <- 0
  pop$last.Tinf <- 0
  ...
  eRates <- list()
  # the Rate of the next event(based on beta or gamma, depending on which
  # compartment the individual is in)
  for (k in 1:ngroups)
      eRates[[k]] <- rep(NA, nr = gr.size)
  ...
  for (i in 1:ngroups) {
      ...
      # end: infected to be recovered==0 or susceptible to be infected==0
      while (sum(pop.gr$I) > 0 & sum(pop.gr$I == 0) > 0) {
          # event rates
          for (j in 1:gr.size) {
              # (recovered individuals cannot be selected, if recovered R==1 and
              # then it can't be selected eRates==0)
              if (pop.gr$R[j] == 1)
                  eRates[[i]][j] = 0   
              else{
                  if (pop.gr$I[j] == 0) {
                      # if the individual is not infected, rate of infection for
                      # each susceptible
                      if (reprnum)
                          eRates[[i]][j] = exp(pop.gr$g[j]) * (TRbeta / freqDepTerm) * exp(pop.gr[pop.gr$s == 0, ]$f)
                      else
                          eRates[[i]][j] = exp(pop.gr$g[j]) * (TRbeta / freqDepTerm) * sum(exp(pop.gr[pop.gr$I == 1, ]$f)) 
                  }  else {                       
                      # constant recovery rate of each infected (no variation)
                      eRates[[i]][j] = RRgamma
                  }
              }  
          }
          ...
          #data with number of SIR over time
          SIRts[[i]] <- rbind(SIRts[[i]], SIRts[[i]][nrow(SIRts[[i]]), ])
          jj <- nrow(SIRts[[i]])
          
          # to avoid that the first event if recovered of index case
          # ll > 1
          
          if (pop.gr[pop.gr$ID == IDnextT, ]$I == 1) {
              # recovery of an infected

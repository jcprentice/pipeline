...
discretise <- function(pop, pars) {
    
    
    interval_duration <- pars$interval_duration
    ...
    if (epi_type == "SIR") {
        if (interval_duration == "event") {
            popvv <- pop[, .(id, group, Tinf2, Trec2, Tinf, Trec, censored, index_case)][dt1]
            pop[, Tinfind := Tinf2]
            pop[, Trecind := Trec2]
            ...
            popvv <- pop[, .(id, group, Tinf2, Trec2, Tinf, Trec, censored, index_case, int.duration)][dt1]
        }
    } else {
        if (interval_duration == "event") {
            popvv <- pop[, .(id, group, Tinf2, Tinf, censored, index_case)][dt1]
            pop[, Tinfind := Tinf2]
            setkey(pop, group, Tinfind)
            setkey(popvv, group, Tinfind)
            popvv <- pop[, .(group, Tinfind, int.duration)][popvv]
        } else {
            popvv <- pop[, .(id, group, Tinf2, Tinf, censored, index_case, int.duration)][dt1]
        }
    }
    ...
    # }
    # 
    # if (epi_type != "SIR" & interval_duration == "event") {
    #     popvv <- pop[, .(id, group, Tinf2, Tinf, censored, index_case)][dt1]
    #     pop[, Tinfind := Tinf2]
    #     setkey(pop, group, Tinfind)
    #     setkey(popvv, group, Tinfind)

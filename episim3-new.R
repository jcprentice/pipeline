EpiSim3_new <- function(
    ...
    if (is.null(group.and.index)) {
        ...
        if (Epi.type == "SI") {
            SIRts <- data.table(group = unique(pop[, group]), time = 0)
    ...
    # while loop for SI model should stop once everyone is infected as well as
    # if no-one is infected. For SIS, while loop stops when no-one is infected
    # (**could go on a long time!!**). Current while loop should be able to
    # accommodate SIS model, but need another stopping argument.
    ...
    if (Epi.type == "SIR") {
        ...
        while (SIRts[, last(totI) > 0]) { # & last(totI) < nind])
            ...
            print(prob)
            IDnextT <- sample(pop[, id], size = 1, prob = prob)
